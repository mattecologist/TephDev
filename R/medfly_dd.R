...
medfly_dd <- function (temps=temps, 
  ...
  begin.date <- as.Date(begin.date, origin="1970-01-01")
      ...
      larva <- cbind(day_degree_simple(11.1, 100, temps, start.date=min(egg$Date[egg$CDD > 44])), "Stage" ="larva")
      pupa <- cbind(day_degree_simple(8.4, 100, temps, start.date=min(larva$Date[larva$CDD > 162])), "Stage" ="pupa")
      adult <- cbind(day_degree_simple(12.8, 100, temps, start.date=min(pupa$Date[pupa$CDD > 56])), "Stage" ="adult")
